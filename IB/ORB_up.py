...
# LIVE ACCOUNT 
#
#ib_acct = "U6408055"  # update the ib account (different from real account and paper account)
#
#
tickers = ["PSFE","SOFI","LCID"]  # pick tickers with highest gap up or gap down
...
def kill_switch(app):
    ...
    for ticker in pos_df["Symbol"]:
        ...
        if quantity > 0:
            app.placeOrder(order_id, usStk(ticker),
                           marketOrder("SELL", quantity))  # EClient function to request contract details
        if quantity < 0:
...
def openRangeBrkout(app):
    while not kill_event.is_set():
        for ticker in tickers:
            ...
            if current_tot_pnl > profit_limit or current_tot_pnl < loss_limit or (
                    (int(hour) >= 22 and int(minute) > 30)):
                kill_event.set()
                kill_switch(app)
                continue
...
strategyThread = threading.Thread(target=openRangeBrkout, args=(app,))
