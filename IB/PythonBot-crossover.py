...
class Bot:
    ...
    smaPeriod = 50
    symbol = ""
    initialbartime = datetime.now().astimezone(pytz.timezone("US/Pacific"))
    ...
    def __init__(self):
        ...
        if (int(self.barsize) > 1):
            mintext = " mins"
        queryTime = (datetime.now().astimezone(pytz.timezone("US/Pacific")) - timedelta(days=1)).replace(hour=16,
                                                                                                               minute=0,
                                                                                                               second=0,
    ...
    def on_bar_update(self, reqId, bar, realtime):
        ...
        if (realtime == False):
            ...
            bartime = datetime.strptime(bar.date, "%Y%m%d %H:%M:%S").astimezone(pytz.timezone("US/Pacific"))
            ...
            if (minutes_diff > 0 and math.floor(minutes_diff) % self.barsize == 0):
                ...
                self.sma = ta.trend.SMAIndicator(self.close, self.window, True)
